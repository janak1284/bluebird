...
NODE_ENDPOINT_MAP = {
    "willson":       {"ip": "127.0.0.1"},
    "core-master":   {"ip": "127.0.0.1"},
    "fedora":        {"ip": "10.243.176.218"},
    "edge-node-01":  {"ip": "10.243.176.218"},
    "desktop-prnd0ve": {"ip": "10.243.176.77"},
    "edge-node-02":    {"ip": "10.243.176.77"},
...
ROUTER_UPDATE_URL = "http://localhost:8000/api/v1/router/update"
...
def update_router_table(route, new_node, new_target):
    """Notifies custom router.py on Port 8000 to update the live route table."""
    try:
        payload = json.dumps({
            "route": route,
            "new_node": new_node,
        ...
        print(f"⚠️ Router update notification failed: {e}")
        return False
...
def execute_make_before_break_migration(workload_name, target_node):
    ...
    # 2. Wait for new pod on target_node to reach Ready == True (Make-Before-Break Health Gate)
    new_pod_ready = False
    timeout_sec = 30
    ...
    if success:
        print(f"  [3/4] Custom Router Table updated: '{route_path}' ➔ {new_target_url}")
    else:
        print(f"  [3/4] ⚠️ Custom router update failed, but K3s NodePort remains accessible.")
        
    # 4. K3s automatically tears down old pod in background
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        # Direct CLI usage: python3 scheduler.py <workload_name> <target_node>
        w_name = sys.argv[1]
        t_node = sys.argv[2]
        apply_placement({w_name: t_node}, explanation=f"Manual CLI migration request to {t_node}")
    elif len(sys.argv) == 2:
        # JSON payload CLI usage: python3 scheduler.py '{"checkout-critical-01": "fedora"}'
        try:
        ...
        print("Usage: python3 scheduler.py <workload_name> <target_node>")
        print("Example: python3 scheduler.py checkout-critical-01 fedora")
